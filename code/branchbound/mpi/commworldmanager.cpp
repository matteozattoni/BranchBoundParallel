...
CommWorldManager::CommWorldManager(MPIDataManager &manager) : ParallelManager(), dataManager(manager)
{
    int worldSize;
    int worldRank;
    MPI_Comm_size(MPI_COMM_WORLD, &worldSize);
    MPI_Comm_rank(MPI_COMM_WORLD, &worldRank);
    masterpoolComm = MPI_COMM_WORLD;
...
CommWorldManager::~CommWorldManager()
{
    if (masterpoolComm != MPI_COMM_NULL)
    {
        checkTermination();
...
BranchBoundProblem *CommWorldManager::getBranchProblem()
{
    throw MPIUnimplementedException("MPIGlobalManager::getBranchProblem()");
}

Branch *CommWorldManager::getRootBranch()
{
    throw MPIUnimplementedException("MPIGlobalManager::getRootBranch()");
}

BranchBoundResultBranch *CommWorldManager::getBranch()
{
...
BranchBoundResultBranch *CommWorldManager::waitForBranch()
{
    try
    {
        return getBranch();
...
void CommWorldManager::prologue(std::function<void(BranchBoundResult *)> callback)
{
    if (masterpoolComm == MPI_COMM_NULL || masterpoolSize < 2)
        return;
...
void CommWorldManager::epilogue(std::function<const Branch *()> callback)
{
...
void CommWorldManager::sendBound(BranchBoundResultSolution *bound)
{
...
void CommWorldManager::sendToken()
{
    if (!tokenTermination.hasToken)
        return;
...
double CommWorldManager::getBound()
{
    return bound;
}

Branch *CommWorldManager::returnBranchFromStatus(MPI_Status status)
{
    int count;
...
bool CommWorldManager::isCommEnabled()
{
    return masterpoolComm != MPI_COMM_NULL;
}

void CommWorldManager::broadcastTerminationWithValue(bool value)
{
}

void CommWorldManager::checkTermination()
{
    int someMessage;
    MPI_Status status;
...
Branch *CommWorldManager::getBranchFromGeneralStatus(MPI_Status status)
{
...
bool CommWorldManager::isLocalTerminate()
{
    if (ringBranchReceived.request != MPI_REQUEST_NULL)
        return false;
    /*     if (branchSent.request != MPI_REQUEST_NULL)
...
void CommWorldManager::terminate()
{
...
int CommWorldManager::getIdentity() {
